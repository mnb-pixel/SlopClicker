...
export function GoldenMemeBanner({ activeEvent, dismissEvent, adState, requestBonus, isAdReady, adFree = false, t, tf }) {
  // Zwei Looks, ein Markup - siehe src/components/skin.jsx. Im Spiel-Skin ist das Banner
  // ein Aushang aus Papier mit goldener bzw. roter Kante, passend zur Insel darunter,
  // statt einer dunklen Neon-Leiste über dem hellen Himmel.
  const { cx } = useSkin();
  ...
  // Dieselbe Aussage im Spiel-Skin: Glut-Rot für den Blasen-Debuff, Gold für das Meme.
  const gsAcc = isBubble ? 'gs-acc-rust' : 'gs-acc-gold';
  ...
  return (
    <div className="fixed banner-safe-top left-1/2 -translate-x-1/2 z-50 w-full max-w-xl px-3 pointer-events-auto">
      <div className={cx(
        `w-full p-3 md:p-4 rounded-2xl border-4 shadow-2xl golden-meme-pulse ${accent.card}`,
        `gs-skin gs-panel gs-banner ${gsAcc} golden-meme-pulse`
      )}>
        ...
        <div className="flex items-start gap-3">
          <div className={cx(`p-2.5 rounded-full shrink-0 ${accent.icon}`, `gs-medal ${gsAcc}`)}>
            {isBubble ? <ShieldAlert className="w-5 h-5" /> : isClaimed ? <Zap className="w-5 h-5" /> : <Sparkles className="w-5 h-5" />}
          </div>

          <div className="min-w-0 flex-1">
            <div className={cx(
              `font-black text-sm md:text-base uppercase tracking-wide break-words ${accent.title}`,
              'gs-title text-sm break-words'
            )}>
              {title}
            </div>
            <div className={cx(
              'text-[11px] md:text-sm font-medium opacity-90 text-slate-200 mt-0.5 break-words',
              'gs-sub mt-0.5 break-words'
            )}>
          ...
          <button
            ...
            className={cx(
              'shrink-0 p-1.5 -m-1 rounded-lg text-slate-300/70 hover:text-slate-100 hover:bg-slate-950/40 transition-colors',
              'gs-iconbtn'
            )}
          >
            <X className="w-4 h-4" />
        ...
        <div className="mt-2.5">
          <div className={cx(
            'flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-slate-300/80 mb-1',
            'flex items-center justify-between gap-2 gs-stat__label mb-1'
          )}>
          ...
          <div className={cx('h-1.5 w-full rounded-full bg-slate-950/50 overflow-hidden', `gs-meter ${gsAcc} w-full`)}>
            <div
              className={cx(
                `h-full rounded-full transition-[width] duration-200 ease-linear ${accent.bar}`,
                'gs-meter__fill'
              )}
        ...
        {showAdButton && (
          <button
            ...
            className={cx(
              `mt-2.5 w-full py-3 rounded-xl font-black text-xs md:text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-lg ${
                isAdPlaying
              ...
              `gs-btn gs-btn--block mt-2.5 py-3 ${
                isAdPlaying ? 'is-disabled animate-pulse' : adReady ? (isBubble ? 'gs-btn--danger' : 'gs-btn--gold') : 'is-disabled'
              }`
            )}
          >
        ...
        {!isBubble && !isClaimed && (
          <div className={cx(
            'mt-1.5 text-center text-[10px] font-bold uppercase tracking-wider text-slate-400',
            'gs-stat__label text-center mt-1.5'
          )}>
